...
### 实践5：纯NumPy实现Self-Attention

```python
import numpy as np

print("=" * 60)
print("实践5: 纯NumPy实现Self-Attention (无PyTorch依赖)")
print("=" * 60)
...
def numpy_self_attention(X, W_Q, W_K, W_V, W_O):
    """
    纯NumPy实现Self-Attention

    Args:
        X: (batch, seq_len, d_model) - 输入
        W_Q, W_K, W_V, W_O: (d_model, d_model) - 权重矩阵
    Returns:
    ...
    # 注意力分数
    d_k = Q.shape[-1]
    scores = Q @ np.transpose(X, (0, 2, 1)) / np.sqrt(d_k)
    ...
    # Softmax
    attn_weights = numpy_softmax(scores, axis=-1)
...
def numpy_multi_head_attention(X, W_Q, W_K, W_V, W_O, num_heads):
    """
    纯NumPy实现多头注意力
    """
    ...
    for h in range(num_heads):
        ...
        out, weights = numpy_self_attention(
            X[:, :, sl],
            W_Q[sl, sl], W_K[sl, sl], W_V[sl, sl], W_O[sl, sl]
        )
        outputs.append(out)
    ...
    return multi_head_out, np.stack(all_weights, axis=1)
...
# 单头注意力
output, weights = numpy_self_attention(X, W_Q, W_K, W_V, W_O)
print(f"输入 shape: {X.shape}")
print(f"输出 shape: {output.shape}")
print(f"注意力权重 shape: {weights.shape}")
print(f"每行之和: {weights[0].sum(axis=1).round(6)}")
print(f"因果掩码验证: {np.allclose(weights[0][np.triu_indices(seq_len, k=1)], 0)}")

# 多头注意力
mha_out, mha_weights = numpy_multi_head_attention(X, W_Q, W_K, W_V, W_O, num_heads)
print(f"\n多头注意力输出 shape: {mha_out.shape}")
print(f"多头注意力权重 shape: {mha_weights.shape} (batch, heads, seq, seq)")

# 可视化
tokens = [f"t{i}" for i in range(seq_len)]
print(f"\n第一个batch第一个头的注意力权重:")
print(f"{'':>6s}", "".join(f"{t:>6s}" for t in tokens))
for i in range(min(5, seq_len)):
    row = "  ".join(f"{mha_weights[0,0,i,j]:6.3f}" for j in range(min(5, seq_len)))
    print(f"{tokens[i]:>6s} [{row}]")
```
